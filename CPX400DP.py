...
class CPX400DP:
    ...
    def connect(self, port: str, baudrate: int = 9600) -> None:
        ...
        Parameters:
        - port (str): The name or identifier of the serial port.
        - baudrate (int, optional): The baud rate for the serial connection. Default is 9600.
        
        Raises:
        - Exception: If an error occurs while connecting to the specified port.
        """
        try:
            logger.debug(f'Connecting to "{port}" with speed "{baudrate}"...')
            self.serialConnection = serial.Serial(port, baudrate)
            logger.debug(f'Connected to "{port}" with speed "{baudrate}".')
        except Exception as e:
            logger.exception(f'An error occurred while connecting to port due to {repr(e)}.')
            raise e
    ...
    def autoConnect(self) -> None:
        ...
        Raises:
        - ValueError: If no CPX400DP supplies are detected or if multiple supplies are detected without specifying a specific com port.
        - Exception: If an error occurs during the automatic connection process.
        """
        try:
            logger.debug('Automatically connecting to device...')
            ...
            match power_supply_connected_count:
                ...
                case 1:
                    ...
                    port = power_supply_comport.device
                    self.connect(port)
                case _:
            ...
            logger.error(f'An error occurred while automatically connecting to device due to {repr(e)}')
            raise e
    ...
    def disconnect(self) -> None:
        ...
        """
        try:
            logger.debug(f'Terminating connection...')
            self.serialConnection.close()
            logger.debug(f'Terminated connection.')
        except Exception as e:
            logger.exception(f'An error occured while terminating connection due to {repr(e)}.')
            raise e
    ...
    def setVoltage(self, voltage: float, channel: int = 1) -> None:
        ...
        """
        try:
            logger.debug(f'Setting voltage for channel {channel} to {voltage}V...')
            self.serialConnection.write(bytes(f'V{channel} {voltage}\n'.encode('utf-8')))
            logger.debug(f'Set voltage for channel {channel} to {voltage}V.')
        except Exception as e:
            logger.exception(f'An error occured while setting voltage due to {repr(e)}')
            raise e
    ...
    def setCurrent(self, current: float, channel: int = 1) -> None:
        ...
        """
        try:
            logger.debug(f'Setting current for channel {channel} to {current}A...')
            self.serialConnection.write(bytes(f'I{channel} {current}\n'.encode('utf-8')))
            logger.debug(f'Set current for channel {channel} to {current}A.')
        except Exception as e:
            logger.exception(f'An error occured while setting current due to {repr(e)}')
            raise e
    ...
    def enableOutput(self, channel: int = 1) -> None:
        ...
        """
        try:
            logger.debug(f'Enabling channel {channel}...')
            self.serialConnection.write(bytes(f'OP{channel} 1\n'.encode('utf-8')))
            logger.debug(f'Enabled channel {channel}.')
        except Exception as e:
            logger.exception(f'An error occured while enabling channel {channel} due to {repr(e)}')
            raise e
    ...
    def disableOutput(self, channel: int = 1) -> None:
        ...
        """
        try:
            logger.debug(f'Disabling channel {channel}...')
            self.serialConnection.write(bytes(f'OP{channel} 0\n'.encode('utf-8')))
            logger.debug(f'Disabled channel {channel}.')
        except Exception as e:
            logger.exception(f'An error occured while disabling channel {channel} due to {repr(e)}')
            raise e
    ...
    def getSetVoltage(self, channel: int = 1) -> float:
        ...
        """
        try:
            logger.debug(f'Getting set voltage for channel {channel}...')
            self.serialConnection.write(bytes(f'V{channel}?\n'.encode('utf-8')))
            logger.debug(f'Got set voltage for channel {channel}.')
            return float(self.serialConnection.readline()[3:].decode('utf-8').strip())
        except Exception as e:
            logger.exception(f'An error occured while getting set voltage due to {repr(e)}')
            raise e
    ...
    def getSetCurrent(self, channel: int = 1) -> float:
        ...
        """
        try:
            logger.debug(f'Getting set current for channel {channel}...')
            self.serialConnection.write(bytes(f'I{channel}?\n'.encode('utf-8')))
            logger.debug(f'Got set current for channel {channel}.')
            return float(self.serialConnection.readline()[3:].decode('utf-8').strip())
        except Exception as e:
            logger.exception(f'An error occured while getting set current due to {repr(e)}')
            raise e
    ...
    def getOutputVoltage(self, channel: int = 1) -> float:
        ...
        try:
            ...
            logger.debug(f'Got output voltage for channel {channel}.')
            return voltage
        except Exception as e:
            logger.exception(f'An error occured while getting output voltage due to {repr(e)}')
            raise e
    ...
    def getOutputCurrent(self, channel: int = 1) -> float:
        ...
        try:
            ...
            logger.debug(f'Got output current for channel {channel}.')
            return current
        except Exception as e:
            logger.exception(f'An error occured while getting output current due to {repr(e)}')
            raise e
    ...
    def lock(self) -> None:
        ...
        """
        try:
            logger.debug(f'Locking settings...')
            self.serialConnection.write(bytes('IFLOCK\n'.encode('utf-8')))
            logger.debug(f'Locked settings.')
        except Exception as e:
            logger.exception(f'An error occured while locking settings due to {repr(e)}')
    ...
    def unlock(self) -> None:
        ...
        """
        try:
            logger.debug(f'Unlocking settings...')
            self.serialConnection.write(bytes('IFUNLOCK\n'.encode('utf-8')))
            logger.debug(f'Unlocked settings.')
        except Exception as e:
            logger.exception(f'An error occured while unlocking settings due to {repr(e)}')
    ...
    def getIdentification(self) -> typing.Dict[str, str]:
        ...
        """
        try:
            logger.debug(f'Getting identification info...')
            ...
            logger.debug(f'Got identification info.')
            return data_dict
        except Exception as e:
            logger.exception(f'An error occured while getting output current due to {repr(e)}')
            raise e
